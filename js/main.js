// === SLIDERS: START ===
// Initializes all Swiper carousels used across the site.
// Each block targets a specific section (cards, last-minute, testimonials,
// edge/features, facilities, local area, mosaic, property galleries, offers)
// and sets loop/speed, pagination or arrows, and responsive breakpoints.
  ...
  document.querySelectorAll(".faq-cat-arrow").forEach(function (arrow) {
    arrow.addEventListener("click", function () {
      const target = document.querySelector(
        arrow.getAttribute("data-bs-target")
      );
...
// === HMC Carousel in Home Page: START ===
// custom scroller
(function () {
  ...
  if (btnOpen) {
    ...
    btnOpen.addEventListener("click", () => {
      ...
      if (showingMap) {
        ...
        btnOpen.innerHTML =
          '<img src="/Newmans/img/location.png" alt="">  On the map';
      } else {
        wrapList.classList.add("d-none");
        wrapMap.classList.remove("d-none");
        btnOpen.innerHTML =
          '<img src="/Newmans/img/grid.png" alt=""> List view';
    ...
    function loadGoogleMaps(cb) {
      ...
      s.src =
        "https://maps.googleapis.com/maps/api/js?key=AIzaSyDJcq9UOVMoqKLMi2_I8yIwNcX2YAFKNiE&v=weekly&callback=__initFindMapCb";
      s.async = true;
      s.defer = true;
...
document.addEventListener("click", (e) => {
  if (
    e.target.matches(
      ".modalMenu__overlay, .modalMenu__close, .modalMenu__list a"
    )
  ) {
...
// === Format Date Helper: END ===

// === Booking Steps, Tabs & Totals: START ===
// Contains:
// - updateStepsProgress(n): toggles progress dots up to current step
// - updateMobileStepsBar(n): updates mobile step label and title
// - setStep(n): activates step 2–6, updates progress/mobile bar, calls syncPayment/syncConfirmation, scrolls top
// - [data-goto-step] click: navigates to requested step via setStep
// - activatePanel(id): switches tabs (btn/panel), hides all .js-filters
// - tabs .tabs__btn click: triggers activatePanel for selected tab
// - document click (filters): toggles .js-filters via .js-filter-toggle / closes via .js-filters-close
// - result__swiper init: creates local Swiper instances with pagination
// - parseMoney(text): converts "£x,xxx.xx" to number
// - formatMoney(x): formats number to "£x.xx"
// - recalc(): recomputes extras/base/total/today and updates DOM
// - selects change: binds .extra__select to recalc()
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function setStep(n) {
    ...
    if (step5) {
      step5.classList.remove("step--active");
    }
    if (step6) {
      step6.classList.remove("step--active");
    }
    ...
    updateStepsProgress(n);
    updateMobileStepsBar(n);
    ...
    if (toggleBtn) {
      ...
      if (filtersBox) {
        filtersBox.style.display =
          filtersBox.style.display === "block" ? "none" : "block";
      }
      return;
    }
...
// === Booking Steps, Tabs & Totals: END ===


// === Mobile Relocation Utilities: START ===
// Contains:
// - isMobile(): breakpoint check (<768px)
// - relocateResultMore(): move ".result__more" between media/info on mobile/desktop
// - relocateExtraQty(): move ".extra__qty" between body/top depending on viewport
// - ensureAnchorBefore(el): inject hidden placeholder before element (restore point)
// - relocatePageNav(): move ".pagenav" after ".summary" on mobile; restore via anchor on desktop
// - reflowMobileRelocations(): run all relocations in sequence
// - resize listener: debounce + reflow on window resize
// - export: window.reflowMobileRelocations for manual triggering
...
// === Mobile Relocation Utilities: END ===
...
(function () {
  ...
  const startPress = () => {
    pressTimer = setInterval(() => {
      list.scrollBy({ left: 40, behavior: "smooth" });
    }, 120);
  };
